...
This project focuses on handling personal data securely in software applications. It covers topics such as identifying personally identifiable information (PII), implementing log filters to protect sensitive data, encrypting passwords, and authenticating to databases using environment variables. The project aims to teach best practices for safeguarding user information and maintaining data privacy.
...
bob@dylan:~$
bob@dylan:~$ cat 0-main.sql | mysql -uroot -p
Enter password:
bob@dylan:~$
...
bob@dylan:~$
bob@dylan:~$ cat 1-main.sql | mysql -uroot -p
Enter password:
bob@dylan:~$
bob@dylan:~$ echo "SELECT COUNT(*) FROM users;" | mysql -uroot -p my_db
Enter password:
