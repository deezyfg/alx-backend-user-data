...
This project focuses on implementing basic authentication mechanisms in a back-end application for a **Simple API**. **Basic authentication** is a method for an HTTP user agent to provide a username and password with each request to secure access to the application. This project aims to teach the fundamental concepts and techniques required to secure web applications using this authentication method.  Additionally, it covers best practices for securely handling and storing credentials, ensuring that your application remains robust against common security threats.
...
A lightweight HTTP API for interacting with the `User` model.

### Project Structure

#### `models/`
- **`base.py`**: Base class for all API models, handling serialization to file
- **`user.py`**: User model implementation

#### `api/v1/`
- **`app.py`**: Main entry point of the API
- **`views/index.py`**: Basic API endpoints (`/status` and `/stats`)
...
|--------|-------------------------|---------------------------------|
| GET    | `/api/v1/status`        | Retrieve API status             |
...
#### POST `/api/v1/users`
Creates a new user. Required JSON parameters:
- `email`
- `password`
...
#### PUT `/api/v1/users/:id`
Updates an existing user. Required JSON parameters:
- `last_name`
- `first_name`
...
Setup and start the server from the [api/](api/) and [model/](model/) directories included in the project.
